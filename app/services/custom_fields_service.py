...
def update_selection_field_values(
    ...
    try:
        ...
        model_exists = models.execute_kw(
            db, uid, password, 'ir.model', 'search_count',
            [[['model', '=', model_name]]]
        )
        ...
        field_ids = models.execute_kw(
            db, uid, password, 'ir.model.fields', 'search_read',
        ...
        if not new_values_added:
            return {
                ...
                "model": model_name,
                "field": field_name,
                "current_values": [{"value": val, "name": name} for val, name in current_selection_dict.items()]
            }

        # Converte de volta para o formato de seleção esperado pelo Odoo
        updated_selection = [(val, name) for val, name in current_selection_dict.items()]
        ...
        selection_ids = [(0, 0, {'value': val, 'name': name}) for val, name in current_selection_dict.items()]

        try:
            # Primeiro, atualiza a definição do campo com a seleção completa
            result = models.execute_kw(
                db, uid, password, 'ir.model.fields', 'write',
            ...
            try:
                ...
                print(f"Aviso: Não foi possível limpar o cache do ir.model.data: {str(cache_error)}")

            # Tenta atualizar o campo no registry para forçar atualização
            try:
                registry_update = models.execute_kw(
                    db, uid, password, 'ir.model', 'write',
                    [[models.execute_kw(
                ...
                print(f"Aviso: Não foi possível atualizar o registry: {str(reg_error)}")
        ...
        return {
            ...
            "model": model_name,
            "field": field_name,
            "new_values_added": [v.value for v in values if v.value in current_selection_dict],
            "current_values": [{"value": val, "name": name} for val, name in sorted(current_selection_dict.items(), key=lambda x: x[0])]
        }
